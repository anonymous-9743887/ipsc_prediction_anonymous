...
from map_utils import get_mask_rle_iou, get_iou
from utils import col_bgr, resizeAR, add_suffix, drawBox, linux_path
...
def ytvis_to_coco(ytvis_gt, ytvis_preds,
                  ...
                  fix_category_id, incremental):
    coco_gt = coco_preds = None

    if os.path.exists(coco_gt_json_path):
        ...
        for pred in pbar:
            ...
            for idx, segmentation in enumerate(segmentations):
                ...
                if bboxes is not None:
                    ...
                    # pred_mask_img = np.zeros((img_h, img_w, 3), dtype=np.uint8)
                    # pred_mask_img[pred_mask.astype(bool)] = col_bgr['cyan']
                    # drawBox(pred_mask_img, np.asarray(bbox), color='magenta')
                    # pred_mask_img = resizeAR(pred_mask_img, 1280, 720)
                    # cv2.imshow('pred_mask_img', pred_mask_img)
                    # cv2.waitKey(0)
...
def run(params, *argv):
    ...
    images_df = pd.DataFrame.from_dict(gt_json_data['images'])
    ann_df = pd.DataFrame.from_dict(gt_json_data['annotations'])
    pred_df = pd.DataFrame.from_dict(pred_json_data)

    image_ids = list(images_df['id'].unique())

    seq_names = [os.path.dirname(image_id) for image_id in image_ids]
    seq_names = list(set(seq_names))

    n_image_ids = len(image_ids)
    n_seq_names = len(seq_names)

    print(f'n_image_ids:  {n_image_ids}')
    ...
    # images_df_id = images_df.groupby("id").agg(list)
    pred_df_id = pred_df.groupby("image_id").agg(list)

    # assert images_df_id.shape[0] == n_image_ids, "n_image_ids mismatch"

    # pbar = tqdm(images_df_id.iterrows(), ncols=100, total=n_image_ids)
    pbar = tqdm(images_df.iterrows(), ncols=100, total=n_image_ids)
    # pbar = tqdm(image_ids, ncols=100)
    ...
    for image_id, img_info in pbar:

        # img_info = images_df.loc[images_df['id'] == image_id]

        # images_df = images_df.drop(img_info.index)
        # n_df = images_df.shape[0]

        image_id = str(img_info['id'])

        h, w = int(img_info['height']), int(img_info['width'])
        # preds = pred_df.loc[pred_df['image_id'] == image_id]

        try:
            preds = pred_df_id.loc[image_id]
        except KeyError:
            # preds = []
            print(f'no preds for {image_id}')
            continue
        ...
        if params.save:
            ...
            os.makedirs(out_vis_parent_path, exist_ok=1)
        ...
        if vis:
            ...
            anns = ann_df.loc[ann_df['image_id'] == image_id]
            ...
            for ann_id, ann in anns.iterrows():
                ...
                if params.enable_mask:
                    ...
                    # ann_mask_vis = resizeAR(ann_mask_img, width=1280, height=720)
            ...
            # ann_img_vis = resizeAR(ann_img, width=1280, height=720)
        ...
        for pred_obj in pred_objs:
            ...
            csv_seq_name = os.path.dirname(image_id)
            assert csv_seq_name, f"no csv_seq_name found in image_id {image_id}"
        ...
        if vis:
            ...
            if params.show:
                cat_img_vis = resizeAR(cat_img, width=1280)

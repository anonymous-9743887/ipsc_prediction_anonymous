...
def update_track_vis_frame(frame_id, frames, frame_paths, frame_id_to_tracks, seq_name, xml_files, cols,
                           track_vis_frames,
                           track_win_name, save_path=None, vis_size=None, show=False):
...
def get_det_vis_frame(frame_id, all_dets, det_vis_frames, seq_name, seq_path, xml_files, frame_paths, frames,
    ...
    det_vis_frame = vis_dets(frame, dets, enable_mask, header, size=vis_size)
...
def vis_tracks(frame, frame_id, tracks, header, width=0, height=0, cols=None, all_cols=None,
...
def vis_dets(frame, dets, enable_mask, header, width=0, height=0, size=None):
    ...
    dets_vis_frame = annotate_and_show(header, dets_vis_frame, only_annotate=1, n_modules=0)
...
def associate_objects(frame_id, tracks, all_dets, det_ids, method, iou_threshold, vis, frame):
    """
...
def read_xml(file, enable_mask, sources_to_include, sources_to_exclude, img_ext, labels_to_cols):
    ...
    for shape in shapes:
        ...
        try:
            col = labels_to_cols[label]
        except KeyError:
            raise AssertionError('invalid label: {}'.format(label))
...
def get_xml_files(seq_path, inverted, xml_dir='annotations', allow_empty=False):
...
def run(seq_path, params, class_to_color):
    ...
    all_dets = [
        read_xml(file, params.enable_mask, sources_to_include, sources_to_exclude, params.img_ext, class_to_color)
    ...
    if params.sec_start_frame >= 0:
        ...
        while True:
            ...
            ret = get_xml_files(seq_path, params.inverted, xml_dir=xml_dir, allow_empty=True)
            if ret is None:
                break
        ...
        if perform_association:
            ...
            if tracks_to_be_associated and non_target_det_ids:
                ...
                det_to_gt, gt_to_det, unassociated_det_ids, unassociated_track_ids = associate_objects(
                    ...
                    params.method, params.assoc_iou_threshold, 0, frame)
        ...
        if fix_unassoc_track:
            ...
            while True:
                ...
                track_vis_frame = vis_tracks(prev_frame, frame_id - 1, prev_tracks, _header, cols=cols,
                                             size=params.vis_size, header_col='hot_pink')
        ...
        if fix_ambiguous_assoc:
            ...
            while True:
                ...
                track_vis_frame = vis_tracks(frame, frame_id, curr_tracks, _header, cols=cols,
                                             size=params.vis_size, header_col='yellow')
...
def main():
    ...
    for seq_path_id, seq_path in enumerate(seq_paths):
        ...
        if params.save_cell_lines:
            vis_cell_lines(seq_path, all_cell_lines, frames, frame_paths, params.neighborhood, params.ui.vis)


def vis_cell_lines(seq_path, all_paths, frames, frame_paths, neighborhood, vis):
    seq_name = os.path.basename(seq_path)

    pause_after_frame = 0

    for target_id in all_paths:
        track, _paths = all_paths[target_id]
        for _path_id, _path in enumerate(_paths):
            ...
            for _node in _path:
                ...
                header = f'{seq_name} frame {frame_id}: {base_filename} target {target_id} line {_path_id}'

                vis_frame = vis_tracks(frame, frame_id, None, header=None,
                                       cols=[track.col, ], labels=[track.label, ],
                                       track_nodes=[_node, ], target_ids=[target_id, ])
                ...
                vis_frame = annotate_and_show(header, vis_frame, only_annotate=1, n_modules=0)
